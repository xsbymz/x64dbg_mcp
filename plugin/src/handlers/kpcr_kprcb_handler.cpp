...
void register_kpcr_kprcb_routes(c_http_router& router) {
    router.post("/api/kpcr/dump_all_cpus", [](const s_http_request& req) {
        json result;
        SYSTEM_INFO si = {};
        GetSystemInfo(&si);
    ...
    router.post("/api/kpcr/read_prcb_fields", [](const s_http_request& req) {
        json result;
    ...
    router.post("/api/kpcr/detect_dpc_anomalies", [](const s_http_request& req) {
        json result;
