...
void register_heavens_gate_routes(c_http_router& router) {
    router.post("/api/heavens_gate/detect_far_jumps", [](const s_http_request& req) {
    ...
    router.post("/api/heavens_gate/analyze_wow64_transitions", [](const s_http_request& req) {
        json result;
    ...
    router.post("/api/heavens_gate/scan_for_direct_syscalls", [](const s_http_request& req) {
        json body; try { body = json::parse(req.body); } catch(...) { body = json::object(); }
        DWORD targetPid = body.value("pid", (DWORD)GetCurrentProcessId());
        json result;
        result["pid"] = targetPid;
